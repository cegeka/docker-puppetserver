...
# configuration is in /etc/puppet/foreman2.yaml
# this file was added to make it possible to have multiple foreman instances
# to report to.
...
if RbConfig::CONFIG['host_os'] =~ /freebsd|dragonfly/i
  $settings_file ||= '/usr/local/etc/puppet/foreman2.yaml'
else
  $settings_file ||= File.exist?('/etc/puppetlabs/puppet/foreman2.yaml') ? '/etc/puppetlabs/puppet/foreman2.yaml' : '/etc/puppet/foreman2.yaml'
end
